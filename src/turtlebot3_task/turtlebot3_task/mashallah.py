...
Strategy:
  ...
  RELEASE              - reverse to leave the block deposited; if 2 deposits done, finish
  DONE
"""
import math
import rclpy
...
# ---- TUNABLE PARAMETERS ----------------------------------------------------
BLOCK_AREA_MIN          = 500
APPROACH_SPEED_FAR      = 0.17
APPROACH_SPEED_NEAR     = 0.12
...
RELEASE_REVERSE_TIME    = 2.25

SEARCH_ANGULAR_SPEED    = 0.4       # bumped from 0.25 — assumes good lab Wi-Fi
INIT_SPIN_ANGULAR_SPEED = 0.4
ALIGN_GAIN              = 0.005
SAFE_FRONT_STOP         = 0.18

# Total number of misplaced blocks expected (assessment has 1 red + 1 blue misplaced)
EXPECTED_DEPOSITS       = 2

# Search watchdog — if SEARCHING spins this many revolutions without
# spending >1s in APPROACHING, declare done (covers transient false positives)
SEARCH_TIMEOUT_REVS     = 2.0
APPROACH_MIN_VALID_TIME = 1.0       # seconds in APPROACHING before considered "real"

# Survey behaviour — minimum rotation before exiting on "both markers seen"
SURVEY_MIN_RADIANS    = math.radians(90.0)
SURVEY_TIMEOUT_REVS   = 1.5   # if both markers not seen, give up after this many revs
# ----------------------------------------------------------------------------
...
class BlockSorter(Node):
    def __init__(self):
        ...
        # Deposit counter — finish after EXPECTED_DEPOSITS
    ...
    def image_cb(self, msg):
        ...
        if ids is not None:
            ...
            for i, mid in enumerate(ids.flatten()):
                ...
                if self.state in ('INIT_SPIN', 'RESURVEY'):
    ...
    def _both_markers_seen(self):
        """Return True if both red and blue markers have recorded bearings."""
    ...
    def _do_init_spin(self):
        ...
        # Early exit: if we've spun at least the minimum and both markers are seen, done
        ...
        # Timeout: if we've spun a full revolution and still haven't seen both, retry
    ...
    def _do_searching(self):
        ...
        # phase, assume there's nothing left and finish.
        ...
        # Log misplaced detections (throttled)
        for cx, area, *_ in self.red_blocks_pixel:
            bearing = self.block_world_bearing(cx)
    ...
    def _do_approaching(self):
        ...
        if blocks:
            ...
            gain_scale = 1.0 if area < NEAR_AREA_THRESHOLD else 0.5    # was 0.3 — slightly snappier
    ...
    def _do_release(self):
        ...
        if elapsed > RELEASE_REVERSE_TIME:
            ...
            if self.deposits_made >= EXPECTED_DEPOSITS:
                self.get_logger().info('All expected deposits complete. Done.')
                self.state = 'DONE'
                return

            # Reset search watchdog state for the next round
            self.search_spin_accumulated = 0.0
            self.search_spin_last_yaw    = None
            self.approach_enter_time     = None
            self.approach_completed_recently = False

            # Clear bearings so RESURVEY records fresh ones from the new position
            self.marker_world_bearings   = {}

            # Reset resurvey spin trackers
            self.resurvey_spin_accumulated = 0.0
            self.resurvey_spin_last_yaw    = None
    ...
    def _do_resurvey(self):
        """Spin until both markers have been freshly seen from the current
        position. Required after a deposit because the robot's perspective
        on each marker changes once it's moved away from the centre.
        Same minimum-spin / early-exit logic as INIT_SPIN."""
        accum = self._accumulate_yaw_change('resurvey_spin_last_yaw', 'resurvey_spin_accumulated')

        # Early exit: minimum spin done AND both markers seen
        if accum >= SURVEY_MIN_RADIANS and self._both_markers_seen():
            self.get_logger().info(
